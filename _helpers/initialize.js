...
async function initializeData(db) {
    try {
        ...
        `);
        // If no subjects found, insert the initial data
        ...
        const [accountResults] = await db.sequelize.query(`
            SELECT COUNT(*) as count FROM \`accounts\` WHERE \`id\` IN (1, 2, 3, 4);
        `);
